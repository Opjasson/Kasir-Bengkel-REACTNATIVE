...
const ProsesTransaksi: React.FC<props> = ({ navigation, route }) => {
    ...
    const createTransaksi = async () => {
        await fetch(`http://192.168.200.220:5000/transaksi/${transaksiId}`, {
            method: "PATCH",
            headers: {
        ...
        cart.forEach(async (item: any) => {
            await fetch("http://192.168.200.220:5000/cart", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
        ...
        cart.forEach(async (item: any) => {
            ...
            if (foundBarang) {
                await fetch(`http://192.168.200.220:5000/barang/${item.id}`, {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json",
                    },
